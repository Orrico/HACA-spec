...
The CPE exposes one internal mechanism: **internal reasoning mode**. The CPE MAY invoke it autonomously when a reasoning step requires unconstrained deliberation — the model is called without persona constraints for that specific inference. The output MUST be processed through normal persona-constrained reasoning before generating any intent payload. No output of an internal reasoning invocation SHALL leave the CPE without passing through the persona layer. The conditions under which internal reasoning mode may be used are further constrained by the active Cognitive Profile.
...
**Internal Reasoning Mode** — the CPE mechanism that invokes the model without persona constraints for a specific inference, allowing unconstrained deliberation. Output is ephemeral and never leaves the CPE without passing through the persona layer.
