...
**Closure Payload** — a structured package produced by the CPE only at a safe session close, carried in the session-close signal to the integrity layer. Contains three elements: consolidation content (the result of semantic consolidation, written to the Memory Store by the memory layer); the Working Memory declaration (pointer map built by the memory layer after consolidation); and the Session Handoff (a forward-looking record stored in the Memory Store and pointed to by the Working Memory).
