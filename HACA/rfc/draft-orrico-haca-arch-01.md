...
When the integrity layer issues a corrective signal to the CPE in a Degraded state, the CPE MUST execute one or more of the following corrective actions within its own authority: **context reload** — the active context window is discarded and reloaded from the memory layer, clearing any accumulated contamination without ending the session; **cycle reset** — the current Cognitive Cycle is aborted without commit, returning the CPE to a clean state for the next cycle. **session summarization** — the CPE produces a condensed representation of the accumulated Session Store content and signals the MIL to replace the current Session Store with the summarized version, reducing its physical size. After the MIL executes the write, the integrity layer MUST independently re-verify the Session Store size against the threshold `S`. If re-verification confirms the size is within bounds, operation continues; if re-verification fails, the condition MUST escalate to Critical. Session summarization MAY also be applied at normal session close to produce a compact handoff for memory consolidation during the Sleep Cycle. After each corrective action, the integrity layer MUST independently re-verify the condition from outside the CPE. If re-verification fails, the condition MUST escalate to Critical.
...
**Session Summarization** — a CPE operation that produces a condensed representation of accumulated session content. In a mid-session context, it is triggered by the integrity layer when the Session Store exceeds the threshold `S`: the CPE condenses the session content and signals the MIL to replace the Session Store with the summarized version, reducing its physical size. In a session-close context, it is the CPE's natural preparation for memory consolidation: condensing and structuring session content — learnings, preferences, and relevant context — into the form that the MIL consolidates into the Memory Store during the Sleep Cycle.
