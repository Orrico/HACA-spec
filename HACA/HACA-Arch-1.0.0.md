...
When the integrity layer issues a corrective signal to the CPE in a Degraded state, the CPE executes one or more of the following corrective actions within its own authority: **context reload** — the active context window is discarded and reloaded from the memory layer, clearing any accumulated contamination without ending the session; **cycle reset** — the current Cognitive Cycle is aborted without commit, returning the CPE to a clean state for the next cycle. **session summarization** — the CPE produces a condensed representation of the accumulated Session Store content and signals the MIL to replace the current Session Store with the summarized version, reducing its physical size. After the MIL executes the write, the integrity layer independently re-verifies the Session Store size against the threshold `S`. If re-verification confirms the size is within bounds, operation continues; if re-verification fails, the condition escalates to Critical. Session summarization may also be applied at normal session close to produce a compact handoff for memory consolidation during the Sleep Cycle. After each corrective action, the integrity layer independently re-verifies the condition from outside the CPE. If re-verification fails, the condition escalates to Critical.
