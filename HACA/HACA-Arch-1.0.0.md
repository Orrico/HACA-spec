...
Although the Imprint Record resides in the Memory Store, it carries a special operational status: it is the entity's identity anchor — the root from which the Genesis Omega is derived and against which every boot is ultimately verified. For this reason, the Imprint Record is loaded directly by the SIL at every boot as part of the Boot Manifest, independently of the normal mnemonic pipeline. It is the only Memory Store artefact with this property — all other Memory Store content is accessed on demand during the session.
...
The **Working Memory** is a compact, fixed-size artefact produced by the memory layer at the close of every normal Sleep Cycle. It is a navigation map — not a data snapshot: it contains pointers into the Memory Store sufficient for the cognitive engine to resume from where the previous session ended. Its contents are: the reference point in the Memory Store for the last consolidated session, any active background skill handles and their TTL state, and the position of the last completed Cognitive Cycle within the previous session's sequence. The cognitive engine uses the Working Memory to know what to query from the memory layer at session start — not what the answers are. The Working Memory is loaded at boot only when the previous session closed normally through a complete Sleep Cycle. If the previous session ended in a crash, the integrity layer invalidates the Working Memory during crash recovery and it is not loaded — the session starts from the current consolidated state of the Memory Store without resumption context. The size of the Working Memory is fixed and declared in the entity's structural baseline; it does not grow with entity age or operational history.
...
Before dispatching to the two-gate sequence, the EXEC consults the **Skill Index** — a structural artefact loaded at boot that lists every skill authorized for execution. If the requested skill is present in the index, the EXEC proceeds to the first gate. If the skill is absent, the EXEC rejects the payload and signals the MIL to log the rejection without contacting the integrity layer. The Skill Index is described in §4.4.
