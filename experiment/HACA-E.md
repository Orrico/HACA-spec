...
The **Operator Channel** is the integrity system's direct line to the Operator — a permanent escalation path established during the first boot that bypasses the reasoning layer entirely. It is implemented as a mechanism the integrity system invokes directly: a terminal prompt, an email, or any equivalent out-of-band signal. The specific mechanism is defined at implementation time, but the architectural requirement is invariant: no escalation that passes through the reasoning layer can be trusted, since the reasoning layer may itself be the source of the anomaly.
