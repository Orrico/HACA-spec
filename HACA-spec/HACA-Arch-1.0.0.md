...
- **Semantic Drift** — accumulated memory content has diverged from the entity's structural definition. Detection depends on **Semantic Probes**: the entity's semantic drift reference baseline — a set of semantic anchors derived from structural content, maintained by the integrity layer, initialized from the Imprint Record at first activation, and updated only through authorized structural evolution. The MIL uses semantic probes as reference anchors to measure divergence during memory consolidation — a defined comparison operation whose mechanism is implementation-defined, not interpretation of content meaning. **Detected by the memory layer** per Sleep Cycle during memory consolidation. The memory layer signals the integrity layer; the integrity layer classifies and responds. Under HACA-Core, any detected divergence is an anomaly and escalates directly to Critical — the integrity layer has no external indicator by which to independently verify semantic content, so the Degraded path does not apply. Under HACA-Evolve, the integrity layer cross-references the signal against queued Evolution Proposals: divergence covered by an approved Evolution Proposal is legitimate evolution and proceeds to the evolutionary protocol; divergence without a covering proposal is an anomaly.
...
**Integrity** is realized across four concurrent mechanisms. First, the SIL and the EXEC operate as two independent authorization gates on every skill execution: the EXEC requests a manifest integrity check from the SIL, which verifies structural integrity and returns a grant or denial; the EXEC applies its own validation as the second gate. Both gates must pass independently; neither substitutes for the other. Second, the SIL orchestrates the Heartbeat Protocol — continuously monitoring all components, evaluating health signals, independently re-verifying resolution before clearing Degraded states, and escalating to the Operator when anomalies exceed its correction authority or re-verification fails. Third, structural evolution is realized jointly by the MIL and the SIL executing the Endure Protocol during the Sleep Cycle: the atomic commit writes all structural files — including the Integrity Document — as a single indivisible operation, extending the verified chain. Fourth, drift detection is distributed across components: the MIL measures semantic divergence during memory consolidation and signals the integrity layer; the SIL detects identity and evolutionary drift through structural hash comparison and integrity chain analysis. In all cases, classification and response are exclusively the SIL's responsibility.
