...
Each component maintains a **Reciprocal SIL Watchdog**: a local silence counter that increments whenever the expected SIL poll does not arrive within the defined interval. If the counter reaches its threshold — indicating that the SIL has stopped responding — the component activates the Operator Channel directly, without routing through the SIL. This is the only condition under which a component other than the SIL may contact the Operator directly. The escalation records which component detected the SIL silence and at what point in the operational cycle. The Reciprocal SIL Watchdog threshold is defined at implementation time.
...
**Reciprocal SIL Watchdog** — a local silence counter maintained by each component. Increments when the expected SIL poll does not arrive within the defined interval. On threshold: the component activates the Operator Channel directly, bypassing the SIL. The only condition under which a non-SIL component may contact the Operator directly.
