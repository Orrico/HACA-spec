...
The integrity layer controls the write lock and drives each stage in sequence. The memory layer executes the physical atomic commit when instructed by the integrity layer; the integrity layer then updates the Integrity Document to extend the verified chain. If any step from `atomic commit` onward fails, the integrity layer restores the snapshot and the entity resumes from its last verified structural state. No partial structural evolution is externally visible at any point in the pipeline.
...
The watchdog timer applies exclusively to **synchronous skills** — skills that block the Cognitive Cycle until they return. Skills whose manifest declares them as **background** are exempt from the watchdog. When a background skill is dispatched by the execution layer, it does not block the Cognitive Cycle: the execution layer spawns the execution and immediately returns an **Execution Lease** to the cognitive engine and the memory layer. The Execution Lease is a structured record identifying the running background process, its declared skill, and its initiation timestamp. From this point, the background process is not tracked as a pending step of the active Cognitive Cycle; the cycle proceeds and may close normally. The integrity layer monitors active Execution Leases through the Health Flow, polling each Lease at defined intervals independently of the Pulse Counter. If a Lease reports an abnormal termination — process killed, host signal, unhandled exception — the integrity layer evaluates the failure against the skill's declared criticality. The escalation path for each criticality level is defined by the active Cognitive Profile. The Execution Lease record is persisted by the memory layer and remains in the Session Store until the background process terminates and the integrity layer confirms closure.

The **Drift Framework** defines four categories of behavioral or structural deviation. Detection is distributed: each component detects drift within its own domain and signals the integrity layer. The integrity layer classifies and responds to all drift signals, regardless of origin. The Genesis Omega is the cryptographic root against which all drift is ultimately referenced. Each evolutionary commit extends the integrity chain from that root. Detection criteria, tolerance thresholds, and response procedures for each drift type are defined by the active profile specification.

- **Inference Drift** — an intent payload produced by the cognitive engine contradicts verified persisted state as represented in the cognitive engine's active context window — the state loaded from the memory layer at session start and updated by each completed Cognitive Cycle within the session. **Detected by the cognitive engine** per Cognitive Cycle, before the payload is dispatched. The cognitive engine signals the integrity layer; the integrity layer classifies and responds.
- **Semantic Drift** — accumulated memory content has diverged from the entity's current structural definition as represented in the Integrity Document — the sealed persona, skill manifests, and behavioral parameters that constitute the entity's identity baseline. **Detected by the memory layer** per Sleep Cycle during memory consolidation. The memory layer signals the integrity layer; the integrity layer classifies and responds. Under HACA-Core, any detected divergence is an anomaly. Under HACA-Evolve, the integrity layer cross-references the signal against queued Structural Consolidation Proposals: divergence covered by an approved proposal is legitimate evolution and proceeds to the evolutionary protocol; divergence without a covering proposal is an anomaly.
...
**Volatile Diagnostic Buffer** — a per-cycle ephemeral buffer maintained by the CPE that retains internal reasoning output for the duration of the current Cognitive Cycle. Discarded silently on Nominal cycle close or operational session close. Locked and written to the Integrity Log by the integrity layer exclusively on anomaly-triggered token revocation (Critical state).
